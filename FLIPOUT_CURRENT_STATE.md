...
- Deployed Edge verification passed against `https://flip-md0ub8wpi-chattocal.vercel.app`: held mouse travel **32.125 px**, neighbour preview **−6.817 px**, committed exchange **+63.865/−63.865 px**, capture `yes`, no scroll, and the authoritative board settled from 18 to 17 moves. A 390×844 Chromium touch pass measured **23.434 px** held travel and **+45.929/−43.304 px** committed exchange with no scroll. This is not represented as physical iPhone Safari proof.
...
- Verification: focused UI **3/3**, cloud Match-3 Node **37/37**, cloud UI/token **9/9**, focused lint passed, and the 145-module production build passed. Deployment `dpl_4pSSBn13cpUkvHVmmTCM2z2jqC7f` is Ready. `https://dev.flipout.gizmogames.uk` returned HTTP 200 with the identical HTML SHA-256 and `assets/index-BMjYvOQx.js` entry. Production was not touched.
