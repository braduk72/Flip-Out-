...
- **Status: implemented locally; Preview deployment pending in this milestone.** Additive migration `014_theme_albums.sql` creates immutable official Theme Album entries, collector-achievement records and idempotent album transaction receipts.
...
- Verification so far: focused Theme Album/Collection Node tests **10 passed / 1 Preview DB skip**; focused Collection UI tests **8 passed**; changed-file lint passed; full local suite **129 Node passed / 12 expected Preview-only skips** and **50 UI passed**; production build passed with **152 transformed modules**. Preview DB migration/test/deployment still need to run before this milestone is complete.
