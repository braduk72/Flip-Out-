...
- Preview deployment verification is not complete. `npx.cmd vercel deploy --yes --scope chattocal --local-config vercel.inventory-capacity-verify.json` and a forced retry both timed out locally while Vercel left Preview deployments in `QUEUED`. The stuck queued deployments `flip-q6dgi12es-chattocal.vercel.app`, `flip-cbdp99o79-chattocal.vercel.app` and `flip-6yrlaxzte-chattocal.vercel.app` were removed. `vercel inspect --wait --timeout 180s` still reported `status ● Queued` before removal.
- A fallback `vercel build --target=preview --scope chattocal --local-config vercel.inventory-capacity-verify.json` could not run locally because Vercel CLI failed before project code with `spawn cmd.exe ENOENT`. `vercel env ls preview --scope chattocal` confirms encrypted `DATABASE_URL` still exists for Preview branch `dev`, but `vercel env pull` does not expose it locally, so the migration cannot be safely applied outside a real Vercel Preview/dev build.
- `https://dev.flipout.gizmogames.uk` still returns HTTP 200 but serves the previous `assets/index-Bg_oms7A.js` bundle containing `1.7.0-personal-albums`, not `1.8.0-inventory-capacity`. Production was not touched.
- Remaining risks: `016_inventory_capacity.sql` has not yet been confirmed applied to the Preview database; the new Preview DB tests have not yet run against Railway. Do not continue to booster grants, Exchange, Recycler rewrite or other economy features until a Ready Preview deployment applies and verifies this boundary.
