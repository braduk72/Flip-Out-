...
- Verification: focused Match-3 Node **40/40** passed; focused Match-3/Settings UI **11/11** passed; focused ESLint on touched files passed; `npm.cmd run test` passed with Node **175 passed / 20 expected Preview-only skips** and UI **67/67**; `npm.cmd run test:preview` locally skipped all **20** DB tests because no local Preview `DATABASE_URL` is exposed; production build passed with **160 modules**; Preview build passed with **160 modules**. Broad source lint still fails with **17 pre-existing legacy errors** in `src/App.jsx`, `src/screens/Game.jsx`, `src/screens/MultiplayerLobby.jsx`, `src/data/decks.js`, `src/data/seasonalOpponents.js` and `src/utils/gameStorage.js`.
