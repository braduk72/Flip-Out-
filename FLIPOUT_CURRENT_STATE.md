...
- **Status: Implemented and verified on development Preview.** Reward Theatre is no longer hard-coded as an animated reel/slot-machine presentation. The reward backend remains authoritative and commits the reward before the client receives presentation data.
...
- Preview deployment: commit `92a851c` deployed Ready as `dpl_4HXi31D9XxcBrGGFfAmASzAmKjcR`, generated URL `https://flip-l1pnx6gqr-chattocal.vercel.app`, target Preview. `https://dev.flipout.gizmogames.uk` returned HTTP 200 with matching ETag `"d4cdace75627a431067f10335c450455"` and byte-identical HTML SHA-256 `0CCDB902F74F6B00F8D55CE5FA68CA9344A37544C710745B3D751E5C52F62788`. Entry bundle `/assets/index-CfieckUb.js` contains `1.22.0-reward-theatre-wheel`; lazy Match-3 bundle `/assets/Match3-BMbhU_p3.js` contains `lucky-prize-wheel`; Match-3 CSS `/assets/Match3-Bua61NmA.css` contains `prizeWheelSpin`, does not contain `reelSpin`, and did not match the theatre/wheel infinite-animation guard.
- Browser smoke: permanent dev URL rendered Home with build version `1.22.0-reward-theatre-wheel` at 390×844 and 1024×768, with no horizontal overflow.
...
- What changed: added deterministic objective-aware Match-3 move hints after 30 seconds of a settled idle board; any click, drag, keyboard input, board change, animation or power-up interaction resets the hint. Added Settings controls for Move Hints, Animation intensity (`full`/`reduced`/`instant`) and Screen Shake.
- No-moves presentation: server-authoritative dead-board reshuffles are now surfaced through presentation metadata as a visible `No more moves` shuffle phase instead of silently accepting a teleported board. Player-initiated shuffle power-ups also show a visible `Board shuffled` phase.
- Cascade multiplier: display now says `Cascade chain` and reports only cascade matches after the player’s original move, while preserving existing engine score calculation and level balance.
- Files changed: `src/match3/hints.js`, `src/match3/presentation.js`, `src/match3/effects.js`, `src/screens/Match3.jsx`, `src/screens/Match3.module.css`, `src/screens/Settings.jsx`, `src/screens/Settings.module.css`, `src/ui/motion.js`, `src/utils/playerSettings.js`, `src/version.js`, focused Match-3/Settings tests.
- Broad lint cleanup: after the first Preview verification, the remaining **17 pre-existing legacy source-lint errors** were repaired without changing gameplay/economy rules. This touched `src/App.jsx`, `src/screens/Game.jsx`, `src/screens/MultiplayerLobby.jsx`, `src/data/decks.js`, `src/data/seasonalOpponents.js` and `src/utils/gameStorage.js`.
- Verification: focused Match-3 Node **40/40** passed; focused Match-3/Settings UI **11/11** passed; focused ESLint on touched files passed; broad `npm.cmd run lint:source -- --quiet` passed; `npm.cmd run test` passed with Node **175 passed / 20 expected Preview-only skips** and UI **67/67**; `npm.cmd run test:preview` locally skipped all **20** DB tests because no local Preview `DATABASE_URL` is exposed; production build passed with **160 modules**; Preview build passed with **160 modules**.
- Preview deployment: Match-3 polish first deployed Ready as `dpl_GKMMdVFUern3fgNiV94Fb8vxLd8y`; after report and lint cleanup commits, `https://dev.flipout.gizmogames.uk` now inspects as Ready deployment `dpl_Af3oXiG2Vamf6ETiFAPikDaQmauj`, generated URL `https://flip-9pqbgbshz-chattocal.vercel.app`, target Preview. `https://dev.flipout.gizmogames.uk` returned HTTP 200 with the same ETag `"877c48c9aa19c753994b4f17a64786f2"` and byte-identical HTML SHA-256 `80A7E2157FF7D414F9016E3B599A93621E7069BF754F510B833D8224064E7E10`. Entry bundle `/assets/index-DbS1GjYG.js` contains `1.21.0-rc-match3-polish`; lazy Match-3 bundle `/assets/Match3-CtEHZd6J.js` contains `No more moves`, `Cascade chain` and `hintedTile`.
- Remaining risks: physical iPhone/Android feel for the 30-second hint pulse, shuffle timing and screen-shake strength still requires human device QA. This slice did not change level balance, reward economy, power-up grants, missions, foils, boosters or Exchange logic. Production was not touched.
...
- Economy/progression boundary: Season Score is earned from gameplay events; Season Tickets are created only from journey level-ups, including levels beyond 100; reward choices spend Tickets transaction-safely; the level-100 Collector Card is awarded once into a separate Season Archive and Inventory; post-100 rewards are repeatable supplies only; mission rerolls are receipt-backed and can later fall back to tokens or Coin costs without direct player-to-player transfer.
...
- Preview deployment: first focused deployment `dpl_H4f5ZAeCYzt3VwEn4arbd1T4EJcy` applied migration `020_season_journey.sql` but failed afterward because an unrelated legacy Match-3 DB cleanup attempted to delete an account referenced by immutable Coin ledger rows. Final focused Season verifier succeeded: `dpl_BpxwqWJJveURgp7sX2Tj1eTEmEfg`, generated URL `https://flip-4k37uf1k1-chattocal.vercel.app`, remote Season tests **8/8**, Vite build **158 modules**. Migration `020_season_journey.sql` is recorded in Railway Preview `railway/public` at `2026-07-20T14:36:50.243Z`.
- Permanent dev URL: `https://dev.flipout.gizmogames.uk` returned HTTP 200 with matching ETag `"c572375bb6a0571cf34460b5a4295266"` and byte-identical HTML as the generated Preview. Main bundle `/assets/index-Bh-D8dbL.js` contains `1.20.0-season-journey`, `service=seasons` and `post100`.
