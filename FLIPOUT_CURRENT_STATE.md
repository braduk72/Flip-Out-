...
- **Status: Implemented locally; Preview deployment verification pending.** This release-candidate hardening slice focused on playability/usability rather than new economy or collection systems.
...
- Verification so far: focused Match-3 Node **40/40** passed; focused Match-3/Settings UI **11/11** passed; focused ESLint on touched files passed; `npm.cmd run test` passed with Node **175 passed / 20 expected Preview-only skips** and UI **67/67**; `npm.cmd run test:preview` locally skipped all **20** DB tests because no local Preview `DATABASE_URL` is exposed; production build passed with **160 modules**; Preview build passed with **160 modules**.
