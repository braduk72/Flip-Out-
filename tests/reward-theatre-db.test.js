...
test('Preview Reward Theatre claims every fifth Match-3 milestone once', { skip: !enabled }, async () => {
  ...
  try {
    ...
    assert.equal(first.presentation.type, 'lucky-prize-wheel')
    assert.equal(first.presentation.wheel.segmentCount, 24)
    assert.equal(first.presentation.wheel.segments.length, 24)
    assert.equal(first.presentation.wheel.segments[first.presentation.wheel.targetIndex].winning, true)
