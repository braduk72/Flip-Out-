...
- Preview deployment: commits `6aa2916` and `2c76e42` were pushed only to `dev`. Application/API verification used Ready deployment `dpl_JAn3NGNQJSLttPuV2x5krL5JwWD2` (`https://flip-dfex0oe42-chattocal.vercel.app`), target `preview`; the permanent URL mapped to it during the claim test. Later report-only commits do not change the verified application bundle or API code.
