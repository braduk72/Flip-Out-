...
export function normaliseHomeData({ playerPayload, match3Payload, liveOpsPayload, seasonStep = 0, storage = globalThis.localStorage } = {}) {
  ...
  return {
    profile: {
      ...
      title: state.playerTitles?.display ?? null,
      titlePrefixId: state.playerTitles?.selected?.prefixId ?? profile.selected_title_prefix_id ?? null,
      titleSuffixId: state.playerTitles?.selected?.suffixId ?? profile.selected_title_suffix_id ?? null,
