...
import { buildCollectionData, filterCollectionCards, readCollectionFavourites, writeCollectionFavourites } from '../ui/collectionData.js'
...
export default function Inventory({ onBack, navProps, dataLoader = playerGameApi.state, actionRunner = playerGameApi.action, storage = globalThis.localStorage }) {
  ...
  useEffect(() => { setVisibleCount(PAGE_SIZE) }, [ownership, query, rarity, setId, sort, variant, view])
  ...
  const resetFilters = () => { setQuery(''); setSetId('all'); setOwnership('all'); setRarity('all'); setVariant('all'); setSort('set') }
      ...
      {status === 'ready' && <>
        ...
        {(view === 'cards' || view === 'favourites') && <section className={styles.cardsView} aria-labelledby="cards-title">
          ...
          <div className={styles.searchRow}>
            <label className={styles.searchBox}><Icon name="search" size={20}/><span className={styles.srOnly}>Search cards and sets</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search cards or sets" /></label>
          ...
          {filtersOpen && <CardPanel variant="inset" className={styles.filterPanel}>
            <label>Set<select value={setId} onChange={event => setSetId(event.target.value)}><option value="all">All sets</option>{collection.sets.map(set => <option key={set.id} value={set.id}>{set.name}</option>)}</select></label>
            <label>Ownership<select value={ownership} onChange={event => setOwnership(event.target.value)}><option value="all">Owned &amp; missing</option><option value="owned">Owned only</option><option value="missing">Missing only</option></select></label>
            <label>Rarity<select value={rarity} onChange={event => setRarity(event.target.value)}><option value="all">All rarities</option>{RARITIES.map(value => <option key={value} value={value}>{value}</option>)}</select></label>
            <label>Variant<select value={variant} onChange={event => setVariant(event.target.value)}><option value="all">All variants</option><option value="base">Base cards</option><option value="gold">Gold Collector</option><option value="foil">Foils</option></select></label>
            <label>Sort<select value={sort} onChange={event => setSort(event.target.value)}><option value="set">Set order</option><option value="name">Name</option><option value="rarity">Rarity</option><option value="newest">Recently obtained</option></select></label>
